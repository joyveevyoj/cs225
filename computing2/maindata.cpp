...
for (int i=0; i<overflowblock.size(); i++){
    if(overflowblock[i]->pri_key==pri_key){return overflowblock[i];}//relation里面需要写get_prikey函数
}
...
while(low<=high&&m<=mainblock.size()){
    mid=(low+high)/2;
    while (mainblock[mid]==tombptr&& mid<=high){mid++;}//if the mid is tombptr, search backward, until find one non-tomb element完了死循环了
    if(mid==high+1){
        mid=(low+high)/2;
...
if(find==NULL){cout<<"cannot delete\n";return dataptr;}//if cannot find, the datapointer returned will have a null pointer in first.
...
if(dataptr.second<mainb_size){return mainblock[dataptr.second];}
else{return overflowblock[dataptr.second-mainb_size];}
}//测试正确
...
for(int i=0; i<over_num; i++){
    mainblock.push_back(overflowblock.back()); overflowblock.pop_back();
    }
//sort mainblock

for (int i=0; i<mainblock.size(); i++){
    if(mainblock[i]==tombptr){
    mainblock.erase(mainblock.begin()+i);
    }//first delete all the tombptr elements
    }
tomb_num=0;
T* k;
int current;
...
//build a new block
 ...
 r_sibling=new_block;
 if(new_block->r_sibling!=NULL){new_block->r_sibling->l_sibling=new_block;}//insert the new block into the doubly linked block list
//reditribution
...
if(r_validnum!=-1&&r_validnum+validnum>2*l_fillfactor*(mainb_size)){
r_sibling->sort();
...
if(l_validnum!=-1&&l_validnum+validnum>2*l_fillfactor*(mainb_size)){
l_sibling->sort();
...
if(r_sibling!=NULL){r_sibling->l_sibling=this;}
 //b树去更新
 for(int i=0; i<mainblock.size(); i++){
     (mainblock[i]->dataptr).first=this;
     (mainblock[i]->dataptr).second=i;//relation类里面需要加dataptr成员
...
l_sibling->sort();
 ...
 if(r_sibling!=NULL){r_sibling->l_sibling=l_sibling;}
...
template <class T, class G, class H>  void block< T,  G,  H>::prettyprint(){
    ...
    else{cout<<"A tombstone\n";}
    }
    cout<<"In the overflowblock:\n";
    for(int i=0; i<overflowblock.size(); i++){
   overflowblock[i]->prettyprint();  
    }
    cout<<"the size of mainblock is:\n";
    cout<<mainblock.size()<<"\n";
    cout<<"the size of overflowblock is:\n";
    cout<<overflowblock.size()<<"\n";
    cout<<"the tombstone num is:\n";
    cout<<tomb_num<<"\n";
