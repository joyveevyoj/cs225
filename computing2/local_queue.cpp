...
string treatment_set[3] = {"Vaccination", "Emergency", "Outpatient"};
...
template<class T>person<T>::person(string tableline)
{
    ...
    //Case when days are in unit digits
    int day, intHour;
    double Minute, Hour;
    if(format_time.length() == 14)
    {
        day = atoi(format_time.substr(7,1).c_str()) - 1;
        intHour = atoi(format_time.substr(9,2).c_str());
        Minute = atof(format_time.substr(12,2).c_str());

    }
    //Case when days are in double digits
    if(format_time.length() == 15)
    {
        day = atoi(format_time.substr(7,2).c_str()) - 1;
        intHour = atoi(format_time.substr(10,2).c_str());
        Minute = atof(format_time.substr(13,2).c_str());
    }
    Hour = Minute / 60 + intHour + day * 24 + month * 28 * 24;
    ...
    //Set the treatment type information
    if(separate_str[14] == "Vaccination")
    {
        treatment = "Vaccination";
        treatment_id = 0;
    }
    if(separate_str[14] == "Emergency")
    {
        treatment = "Emergency";
        treatment_id = 1;
    }
    if(separate_str[14] == "Outpatient")
    {
...
int Timearray[20160];    //Used to avoid time repetition
//0: this time hasn't appeared
//1: this time has appreared
//is in array means has appeared
template<class T> bool person<T>::time_exist(double item)
{
    int index = (int) (10 * item);
...
template<class T>void person<T>::random_generate(int seed)
{
    ...
    //Create Time
    Time = new double[4];
    double newtime = rand() % 2016 + 0.1 * (rand() % 10);  ///2016h is not considered
    while(time_exist(newtime) == true)
    {
        newtime = rand() % 2016 + 0.1 * (rand() % 10);
    }
    ...
    //Create treatment and its id
    treatment_id = rand() % 3;
    treatment = treatment_set[treatment_id];
...
template<class T>string* person<T>::show_format_time()
{
    ...
    for(int i = 0; i < 4;i++)
    {
        ...
        //standardize minute;
        if(Minute[i] != 0 && Minute[i] != 6)
            format_time[i] = format_time[i] + to_string(Minute[i]);
        if(Minute[i] == 6)
            format_time[i] = format_time[i] + "0" + to_string(Minute[i]);
        if(Minute[i] == 0)
...
template<class T>void person<T>::display_all()
{
    ...
    cout << "Treatment ID and Type: " << treatment_id << " " << treatment << endl;
...
string TableWrite::table_line(person<int>& p)
{
    ...
    line = p.id+","+p.name+","+p.address+","+p.phone+","+p.Wechat+","+p.email+","+p.prof+","+p.age+","+p.risk+","+p.show_format_time()[0]+","+to_string(p.withdraw)+","+to_string(p.letter)+","+to_string(p.local_id)+","+hopsital_line+","+p.treatment;
    return line;
}
...
TableWrite::TableWrite(string filename_,int num_,int updated_num_,int withdraw_num_)
{
    ...
    if(num + updated_num + withdraw_num >= 20160)
    {
        cout << "Warning: Too many people, if there are a little more people, earth (three months) can't hold them!" << endl;
        exit(EXIT_FAILURE);
    }
...
void TableWrite::table_create()
{
    ...
    string first_row;
    //Print the first row
    first_row = "ID,Name,Address,Phone,WeChat,Email,Profession,Age,Risk Status,Registration Time,Withdraw,Priority Letter,Local Registry ID,Hopsital ID,Treatment Type\n" ;
    outfile << first_row;
   ...
   //Randomly select updated_num people and update them
    //if no one updates, update no one
