...
template <class T> void Fb_heap<T>::showpriority(){
    person<int>* tempps ;
    vector<person<int>*> pslist ;
    int count =0;
    while(this->getnum()!= 0){
        tempps= delete_min() ;
        pslist.push_back(tempps) ;
        cout<<"This person's ID is "<<tempps->show_id()<< ".The key value is " << tempps->return_key() << endl;   //use deletemin to show priority and insert back to heap
        count ++ ;
    }
    for(int i=0;i<count;i++){
        this->insert(pslist[i]) ;
    }
    cout << endl ;
